          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="i in spuSaleAttrList" :key="i.id">
                ...
                <dd v-for="j in i.spuSaleAttrValueList" :key="j.id" :class="{ active: j.isChecked == 1 }" @click="changeActive(j, i.spuSaleAttrValueList)">{{ j.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" v-model="skuNum" class="itxt" @change="skuNumChange" />
...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      try {
        await this.$store.dispatch('getShopCart', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
        // 路由跳转
        this.$router.push({ name: 'addcartsuccess', query: { skuNum: this.skuNum } })
        // 通过 会话存储 存储当前商品信息
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        // 本地存储和会话存储，一般只存字符串
        // 存储商品信息是先将 对象转化为 json 字符串
      } catch (error) {
        alert(error.message)
      }
    }
